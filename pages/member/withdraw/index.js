...
Page({
  ...
  getInfo: function() {
    ...
    e.get("member/withdraw", {}, function(e) {
      ...
      n.isEmptyObject(e.last_data) || (t.realInfo = {
        ...
        a.setData(t), console.log(a.data.info.data_trade), e.withdrawmoney > 0 && e.credit >= e.withdrawmoney && (t.money = e.withdrawmoney,
          a.moneyChange({
            detail: {
  ...
  jicorechange(e) {
    this.setData({
      jindex: e.currentTarget.dataset.name
    })
  ...
  submit: function(a) {
    ...
    if (!r.disabled && !r.isSubmit)
      if (r.money <= 0) t.toast(i, "请填写提现金额");
      else if (n.isEmptyObject(r.checked)) t.toast(i, "请选择提现方式");
    else {
  ...
  submit_credit: function (a) {
    var i = this,
      r = this.data;
    if (1)
      if (n.isEmptyObject(r.checked)) t.toast(i, "请选择提现方式");
      else {
        var o = r.checked.name,
          d = {
            applytype: r.checked.id,
        ...
        r.info.withdrawcharge > 0 && (c += " 扣除手续费 " + r.deductionmoney + " 元,实际到账金额 " + r.realmoney + " 元"),
          e.confirm(c, function () {
            i.setData({
              isSubmit: !0
            }), e.post("member/withdraw/submit_credit", d, function (a) {
              if (a.error) return t.toast(i, a.message), void i.setData({
                isSubmit: !1
              });
              t.toast(i, "提现申请成功，请等待审核"), setTimeout(function () {
